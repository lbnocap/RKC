...
def func_x2(x,y,yy):
    return (0.1*yy-0.05*math.exp(-0.05*2)*math.sin(x-0.1*2)-0.1*math.exp(-0.05*2)*math.cos(x-0.1*2))/0.05
'''
def func_x2(t, x1, x2):
    v1 = - t ** 2 / 4
    v2 = math.exp(v1)
...
if __name__ == '__main__':
    ...
    # 边界条件
    t = -math.pi
    x2 = math.exp(-0.05*2)*math.cos(t-0.1*2)
    x1 = math.exp(-0.05*2)*math.sin(t-0.1*2)
    x1s, x2s, ts = [], [], []
    # Run
    while t <= math.pi:  # 迭代边界
