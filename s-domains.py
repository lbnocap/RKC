...
for j in range(2,s+1):
         ...
         k[j]=u[j]*k[j-1]+v[j]*k[j-2]+u1[j]*((c[j-1]**2)/2)
print(s2)
...
threerdvalues=(1-x1-x2-x3)+x1*cheb_polys1(Z)/cheb_polys1(w0)+x2*cheb_polys2(Z)/cheb_polys2(w0)+x3*cheb_poly(Z)/cheb_poly(w0)
